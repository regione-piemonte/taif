...
public class SecurityHelper {
	...
	protected it.csi.taif.topforweb.dto.common.UserInfo getCurrentUserInfo(Map session) throws BEException {
		it.csi.taif.topforweb.dto.common.UserInfo userInfo = (it.csi.taif.topforweb.dto.common.UserInfo) session
	...
	public static final Application IRIDE2_APPLICATION = new Application("TOPFORWEB");
	...
	public boolean verifyCurrentUserForUC(Map session, String useCaseCode) throws BEException {
		...
		try {
			it.csi.taif.topforweb.dto.common.UserInfo currentUserInfo = getCurrentUserInfo(session);
	...
	public Identita getCurrUser(Map session) throws BEException {
		Identita id = (Identita) session.get(IRIDE_ID_SESSIONATTR);
		if (id == null) {
			LOG.error(
					"[SecurityHelper::getCurrentUser] Errore nel reperimento del current user dalla sessione: attributo non trovato");
