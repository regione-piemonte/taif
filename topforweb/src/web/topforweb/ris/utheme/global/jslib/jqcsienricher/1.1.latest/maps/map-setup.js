		...
		.Class(
				OpenLayers.Control,
				{
					...
					trigger : function(e) {
						...
						listaParametri[root + ".useCurrentEnvelope"] = true;
						//listaParametri[root + '.mainOffsetLeft'] = leftN;
						//listaParametri[root + '.mainOffsetTop'] = topN;
...
//this.addControllo(new OpenLayers.Control.LayerSwitcher({'div':OpenLayers.Util.getElement('layerswitcher_'+this.idDivMap)}));
					...
					initMap : function(olMapConfig) {
						...
						for ( var jk = 0; olMapConfig.serviziClient != null
							...
							else if (olMapConfig.serviziClient[jk].tipoMapservice == 'GENERICWMS'){
								var wmsVisibleLayers  = "";
								for (var ilay = 0; ilay < olMapConfig.serviziClient[jk].layers.length ; ilay++){
									if (olMapConfig.serviziClient[jk].layers[ilay].visible){
										wmsVisibleLayers = wmsVisibleLayers + olMapConfig.serviziClient[jk].layers[ilay].idLayer +",";
									}
								}
								if (wmsVisibleLayers.endsWith(',')){
									wmsVisibleLayers = wmsVisibleLayers.substring(0,wmsVisibleLayers.length-1);
								}
								var curLayer = new OpenLayers.Layer.WMS(
											olMapConfig.serviziClient[jk].idServizio,
											olMapConfig.serviziClient[jk].url,
											{
											    layers: wmsVisibleLayers,
											    transparent: true
											}, {
											    opacity: olMapConfig.serviziClient[jk].opacity,
											    singleTile: false
											});
								this.addLayer(curLayer);
							}
		...
		.Class(
				OpenLayers.Control.DrawFeature,
				{
					...
					handleEditCommit : function(jsonGeom) {
						...
						var mapDataName = this.map.appDataName;
						/*
							listaParametri[root + '.dimx'] = this.map.getSize().w;
							listaParametri[root + '.dimy'] = this.map.getSize().h;
							listaParametri[root + '.livelloZoom'] = this.map.getZoom();
						...
						//
						var ext = this.map.getExtent();
						listaParametri[mapDataName + '.dimx'] = this.map.getSize().w;
						listaParametri[mapDataName + '.dimy'] = this.map.getSize().h;
						listaParametri[mapDataName + '.livelloZoom'] = this.map.getZoom();
						if (!OOJS.isEmpty(ext)) {
							listaParametri[mapDataName + '.envelope.nordest.coordXStr'] = new String(
									"" + ext.right).replace(",", ".");
							listaParametri[mapDataName + '.envelope.nordest.coordYStr'] = new String(
									"" + ext.top).replace(",", ".");
							listaParametri[mapDataName
									+ '.envelope.sudovest.coordXStr'] = new String(
									"" + ext.left).replace(",", ".");
							listaParametri[mapDataName
									+ '.envelope.sudovest.coordYStr'] = new String(
						...
						//listaParametri['toolbar.comando'] = comando;
						//listaParametri['strIdCurServizio'] = idServizio;
						listaParametri[mapDataName + ".useCurrentEnvelope"] = true;
						//
