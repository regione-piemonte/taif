...
JQCsi.enrichments.AddComboSuggestion = OOJS.extend(
		JQCsi.enrichments.NRichment, {
			...
			applyNRich : function() {
				...
				/*
				var queryParamOgnl = this.appDataName + '.' + this.valueField;
				...
				var store = new Ext.data.JsonStore({
							...
							fields : [this.keyField, this.valueField],
							listeners : {
				...
				if (!OOJS.isEmpty(el)) {
					...
					var suggest = new Ext.form.ComboBox({
								id : this.widgetId,
								hiddenId : 'hidden_'+this.widgetId,
								store : store,
								displayField : this.valueField,
								hiddenName : this.appDataName + '.' + this.keyField,
								valueField : this.keyField,
								submitValue : true,
								queryParam : queryParamOgnl,
								typeAhead : true,
								...
								listeners : {
									render : function(combo){
										if(!Ext.isEmpty(combo.getRawValue())){
											//combo.queryParam = combo.getRawValue();
											//***rileggo il valore selezionato per poter settare l'hidden alla
											//***lettura della pagina se il campo non � vuoto											
											combo.store.reload();
											(function(){
												for (var i = 0; i < combo.store.getCount(); i++) {
												    var rec = combo.store.getAt(i);
													if(!OOJS.isEmpty(rec)){
														var value = rec.get(combo.displayField);
														var idd = rec.get(combo.valueField);
														if(value == combo.getRawValue()){
															document.getElementById(combo.hiddenId).value = idd;
															break;
														}
													}
												}
					...
					if (!OOJS.isEmpty(this.valueChangedUrl)) {					
						suggest.on('select', this.valueChangedCallback);
					}
				}*/
			}
		});
...
JQCsi.enrichments.EditorTextAreaNRichment = OOJS.extend(
		JQCsi.enrichments.NRichment, {
			...
			applyNRich : function() {
				...
				if (el != undefined && el != null) {	// verifico se esiste				
