...
#-------------------------------------------------------------------------------
/*****************************************
 * jqcsienricher v. 1.0.1 - ajaxifyTag   *
 *****************************************/
...
function updateAjaxCallback(id, url, parameters, pageId) {
	...
	for (var i = 0; i < paramSplit.length; i++) {
		if (paramSplit[i] != null && paramSplit[i].length >0){
			var sottoParam = paramSplit[i].split('=');
			lista[sottoParam[0]] = sottoParam[1];
		}
	}
	...
	Ext.Ajax.request({
				...
				params : Ext.apply(lista),
				method : 'POST',
				beforerequest : showLoadBox(),
				// requestcomplete: maskStore.hide(),
				requestexception : Ext.util.Format.messaggiErroreConnection,
				success : function(response, opts) {
					runSuccessAction(response, opts, id, pageId);
				},
