...
#-------------------------------------------------------------------------------
/*****************************************
 * jqcsienricher v. 1.1.3 - ajaxifyTag   *
 *****************************************/
...
function runFailureAction(response, opts) {
	...
	if (response.status == '304') {
		...
		messaggio = "Si e' verificato un errore applicativo.";
	} else {
		messaggio = "Si e' verificato un problema durante la comiunicazione col server: codice errore: HTTP-"+response.status;
	}
	alert(messaggio);
}
...
/**
 * cicla su tutte le istanze di ckeditor e imposta il valore
 * della textarea collegata. deve essere invocata prima di poter utilizzare i parametri
 */
function flushAllVisibleCKEditors() {
	var allCKs = CKEDITOR.instances;
	if (allCKs != undefined && allCKs != null){
		for (var currCKKey in allCKs){
			var currCK = allCKs[currCKKey];
			if (currCK != undefined){
				var tempel = $("#"+currCK.name);
			
				if (tempel != undefined && tempel != null && tempel.length>0){
					var tempData = currCK.getData();
					tempel.val(tempData);
				}
			}
		}
