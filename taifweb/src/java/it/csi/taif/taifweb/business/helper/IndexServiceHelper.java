...
import it.csi.taif.taifweb.business.helper.dto.factory.IndexDtoFactory;
...
public class IndexServiceHelper extends AbstractServiceHelper {
	...
	public String indexCreateRootFolder(MetadatiIndex metadati) throws ServiceException {
		...
		try {
			...
			queryBuilder.append(metadati.getTipoProcedura());
			...
			final Node nodeIdentificativo = this.getEcmEngineWebServiceDelegate().createContent(
					new Node(uidTipoProcedura), this.indexGetContentFolder(metadati.getIdPratica()),
					this.indexGetOperationContext(Constants.INDEX_USERNAME_ADMIN));
	...
	public String indexGetFolder(MetadatiIndex metadati) throws ServiceException {
		...
		try {
			...
			luceneQuery.append(Constants.INDEX_ROOT).append(Constants.INDEX_FOLDER_SUFFIX)
					.append(Constants.TIPOLOGIA_ALBO_IMPRESE);
			
			String uidFolderAifo = this.indexSearchFolder(
					IndexDtoFactory.createSearchParamsForIndexFolder(luceneQuery.toString()), Constants.TIPOLOGIA_ALBO_IMPRESE, null);
			
			luceneQuery.append(Constants.INDEX_FOLDER_SUFFIX);
			luceneQuery.append(metadati.getIdPratica());
			
			String uidFolderPratica = this.indexSearchFolder(
					IndexDtoFactory.createSearchParamsForIndexFolder(luceneQuery.toString()), metadati.getIdPratica(), uidFolderAifo);
			...
			String uidFolderAnno = this.indexSearchFolder(
					IndexDtoFactory.createSearchParamsForIndexFolder(luceneQuery.toString()), annoCorrente, uidFolderPratica);
