...
/*PROTECTED REGION ID(R1029563665) ENABLED START*/
...
public class TaifCnfConfigUtenteDaoImpl extends AbstractDAO implements TaifCnfConfigUtenteDao {
	...
	public void update(TaifCnfConfigUtenteDto dto) throws TaifCnfConfigUtenteDaoException {
		LOG.debug("[TaifCnfConfigUtenteDaoImpl::update] START");
		final String sql = "UPDATE " + getTableName()
				+ " SET FK_PROFILO_UTENTE = :FK_PROFILO_UTENTE ,FK_PERSONA = :FK_PERSONA ,FK_SOGGETTO_GESTORE = :FK_SOGGETTO_GESTORE ,NR_ACCESSI = :NR_ACCESSI ,DATA_PRIMO_ACCESSO = :DATA_PRIMO_ACCESSO ,DATA_ULTIMO_ACCESSO = :DATA_ULTIMO_ACCESSO  WHERE ID_CONFIG_UTENTE = :ID_CONFIG_UTENTE ";

		if (dto.getIdConfigUtente() == null) {
			LOG.error("[TaifCnfConfigUtenteDaoImpl::update] ERROR chiave primaria non impostata");
		...
		LOG.debug("[TaifCnfConfigUtenteDaoImpl::update] END");
	}
	...
	public List<TaifCnfConfigUtenteProfiloByCodiceFiscaleDto> findProfiloByCodiceFiscale(java.lang.String input)
		...
		sql.append("SELECT pu.ID_PROFILO_UTENTE, pu.PROFILO_UTENTE, cu.FK_SOGGETTO_GESTORE, cu.ID_CONFIG_UTENTE");

		sql.append(" FROM TAIF_CNF_CONFIG_UTENTE cu, TAIF_CNF_PROFILO_UTENTE pu, TAIF_T_PERSONA p");

		sql.append(" WHERE ");

		sql.append("cu.FK_PERSONA = p.ID_PERSONA AND cu.FK_PROFILO_UTENTE = pu.ID_PROFILO_UTENTE");

		sql.append(" AND ");

		sql.append("(P.codice_fiscale = :codiceFiscale OR P.n_rsi_msa = :codiceFiscale) AND PU.fk_procedura = 1");
	...
	public List<TaifCnfConfigUtenteProfiloByIdPersonaIdSoggettoGestoreDto> findProfiloByIdPersonaIdSoggettoGestore(
		...
		sql.append(
				"SELECT configUtente.FK_PERSONA as idPersona, configUtente.ID_CONFIG_UTENTE as idConfigUtente, configUtente.FK_SOGGETTO_GESTORE as idSoggettoGestore, configUtente.FK_PROFILO_UTENTE as idProfiloUtente");
		...
		sql.append(
				"configUtente.fk_persona =:idPersona AND configUtente.fk_soggetto_gestore =:idSoggettoGestore AND  configUtente.fk_profilo_utente =:idProfiloUtente AND profiloUtente.fk_procedura = 1");
