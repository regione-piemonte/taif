...
import it.csi.taif.taifweb.util.service.integration.acta.managementservice.ManagementServiceLocator;
import it.csi.taif.taifweb.util.service.integration.acta.managementservice.ManagementServicePort;
import it.csi.taif.taifweb.util.service.integration.acta.smsservice.SMSServiceLocator;
import it.csi.taif.taifweb.util.service.integration.acta.smsservice.SMSServicePort;
...
public class SmsServiceHelper extends AbstractServiceHelper {
	
	private SMSServicePort service;
	...
	public SmsServiceHelper(String urlService) {

		this.urlService = urlService;

	}

	private SMSServicePort getService()
			throws DbManagerException, ServiceException, IOException, ClassNotFoundException {
		LOGGER.debug("[SmsServiceHelper::getService] BEGIN");

		// if (this.service == null) {
		SMSServiceLocator serviceLoc = new SMSServiceLocator();

		SMSServicePort port = serviceLoc.getSMSServicePort();
		...
		this.service = (SMSServicePort) gwfb.create();

		LOGGER.debug("[SmsServiceHelper::getService] Service 'SMSServicePort' INITIALIZED");
		// }

		LOGGER.debug("[SmsServiceHelper::getService] END");
