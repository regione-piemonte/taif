...
public class MenuAction extends BaseAction<GlobalMenuModel> implements ModelDriven<GlobalMenuModel> {
	...
	public Class modelClass() {
		return it.csi.taif.taifweb.dto.GlobalMenuModel.class;
	}
	 ...
	 */
	public String goToMenuNuovaDomanda() throws CommandExecutionException {

		// gestione degli eventi di tipo CLICKED
		ICommand action = initCommand(null, "global", "menuNuovaDomanda", "CLICKED");
		String result = action.doCommand(this);
		if (result != null) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("[MenuAction::goToMenuNuovaDomanda] returning result [" + result + "]");
			}
			setActiveMenu("menuNuovaDomanda");
			return result;
		} else {
			if (LOG.isDebugEnabled()) {
				LOG.debug("[MenuAction::goToMenuNuovaDomanda] returning default result [SUCCESS]");
			}
			setActiveMenu("menuNuovaDomanda");
			return SUCCESS;
		}
	 ...
	 * @return SUCCESS result.
	 */
	public String goToMenuTueDomande() throws CommandExecutionException {

		// gestione degli eventi di tipo CLICKED
		ICommand action = initCommand(null, "global", "menuTueDomande", "CLICKED");
		String result = action.doCommand(this);
		if (result != null) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("[MenuAction::goToMenuTueDomande] returning result [" + result + "]");
			}
			setActiveMenu("menuTueDomande");
			return result;
		} else {
			if (LOG.isDebugEnabled()) {
				LOG.debug("[MenuAction::goToMenuTueDomande] returning default result [SUCCESS]");
			}
			setActiveMenu("menuTueDomande");
			return SUCCESS;
		}
